...
<head>
    ...
    <title>Japanese Yen Transfer</title>
    <style>
        /* Additional styles for the Japanese Yen Transfer page */
        .error-message {
            color: red;
        }
...
<body>
    ...
    <div class="container">
        <h1>Japanese Yen Transfer</h1>

        <!-- Display Japanese Yen account balance -->
        <?php
            ...
            try {
                ...
                // Retrieve current Japanese Yen balance
                $yenStmt = $db->prepare("SELECT balance FROM Currency_account WHERE currency_id = :yenCurrencyId");
                $yenStmt->bindValue(':yenCurrencyId', 4); // Assuming Yen has currency_id 4
                $yenStmt->execute();
                $currentYenBalance = $yenStmt->fetchColumn();

                echo "<p>Japanese Yen Account Balance: $currentYenBalance</p>";
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            try {
                ...
                $conversionRates = [
                    'Euro' => 0.0059, // 0.0059 Yen for 1 Euro
                    'British Pound' => 0.0044, // 0.0044 Yen for 1 Pound
                    'US Dollar' => 0.0065 // 0.0065 Yen for 1 USD
                ];
                ...
                $newYenBalance = $currentYenBalance - $transferAmount;
                $newDestinationBalance = $transferAmount * $conversionRate;

                // Update balances in the database
                $updateYenBalanceStmt = $db->prepare("UPDATE Currency_account SET balance = :newYenBalance WHERE currency_id = :yenCurrencyId");
                $updateYenBalanceStmt->bindValue(':newYenBalance', $newYenBalance);
                $updateYenBalanceStmt->bindValue(':yenCurrencyId', 4); // Assuming Yen has currency_id 4
                $updateYenBalanceStmt->execute();
